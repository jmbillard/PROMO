...
function O_PADEIRO_UTL(thisObj) {
	...
	function buildUi(mainGrp, structureObj) {
		...
		for (var sec in structureObj) {
			...
			if (sectionCounter > 0) {
				var div = mainGrp.add("panel"); // Separador visual
				div.alignment = 'fill'; // Define o alinhamento do painel para preencher o espaço.
			}

			var sectionGrp = mainGrp.add('group'); // Grupo de botões superior
			sectionGrp.alignment = 'center'; // Alinhamento central
			sectionGrp.spacing = 2; // Espaçamento entre botões

			for (var btn in section) {
				var button = section[btn];

				var btnGrp = sectionGrp.add('group'); // Grupo de botões superior
				btnGrp.orientation = 'column'; // Alinhamento central
				btnGrp.alignment = 'center'; // Alinhamento central
				btnGrp.spacing = 0; // Espaçamento entre botões

				var newBtn = btnGrp.add('iconbutton', undefined, button['icon'], { name: 'btn', style: 'toolbutton' }); // Botão "Abrir O Padeiro"
				newBtn.helpTip = button['tip']; // Dica de ajuda
				var newLab = btnGrp.add('statictext', undefined, btn.replace(/_/g, ' '), { name: 'lab', truncate: 'end' }); // Texto do botão
				newLab.justify = 'center'; // Alinhamento do texto
				setTxtHighlight(newLab, '#000000', '#FF7B79'); // Cor de destaque do texto
	...
	function O_PADEIRO_UI() {
		...
		buildUi(mainGrp, PAD_mainGrpUiStructure);
		...
		PAD_ui.apontamento.onClick = function () {
